...
function tsLiteral(value) {
  if (typeof value === "string") {
    return import_typescript.default.factory.createIdentifier(
    ...
    if (options.ctx.enum && schemaObject.enum.every(
      ...
      const metadata = schemaObject.enum.map((_, i) => {
        ...
        return {
          ...
          description: (_f = (_d = schemaObject["x-enum-descriptions"]) == null ? void 0 : _d[i]) != null ? _f : (_e = schemaObject["x-enumDescriptions"]) == null ? void 0 : _e[i]
        };
      });
...
function transformSchemaObjectCore(schemaObject, options) {
    ...
    if (schemaObject.type === "array") {
      ...
      if (schemaObject.prefixItems || Array.isArray(schemaObject.items)) {
        ...
        if ("type" in schemaObject.items && schemaObject.items.type === "array") {
          itemType = import_typescript3.default.factory.createArrayTypeNode(
      ...
      return import_typescript3.default.isTupleTypeNode(itemType) || import_typescript3.default.isArrayTypeNode(itemType) ? itemType : import_typescript3.default.factory.createArrayTypeNode(itemType);
    }
  ...
  for (const k of ["oneOf", "allOf", "anyOf"]) {
    ...
    const discriminator = !schemaObject.discriminator && options.ctx.discriminators.objects[options.path];
    if (discriminator) {
  ...
  for (const paramIn of [
    ...
    if (options.ctx.excludeDeprecated) {
      operationParameters = operationParameters.filter(
        ({ resolved }) => {
          var _a2;
          return !(resolved == null ? void 0 : resolved.deprecated) && !((_a2 = resolved == null ? void 0 : resolved.schema) == null ? void 0 : _a2.deprecated);
        }
      );
    }
