...
function writeIconStyle(node, style, objectStack) {
  ...
  let scale4 = style.getScaleArray()[0];
  let imageSize = size;
  if (imageSize === null) {
  ...
  if (imageSize.length == 2) {
    const resizeScale = scaleForSize(imageSize);
    scale4 = scale4 / resizeScale;
  }
  if (scale4 !== 1) {
    properties["scale"] = scale4;
  }
...
function writeScaleTextNode(node, scale4) {
  writeDecimalTextNode(node, Math.round(scale4 * 1e6) / 1e6);
}
...
// node_modules/ol/node_modules/pbf/index.js
...
var TopoJSON = class extends JSONFeature_default {
  ...
  readFeaturesFromObject(object, options) {
    if (object.type == "Topology") {
      ...
      let transform, scale4 = null, translate3 = null;
      if (topoJSONTopology["transform"]) {
        transform = topoJSONTopology["transform"];
        scale4 = transform["scale"];
        translate3 = transform["translate"];
      }
      const arcs = topoJSONTopology["arcs"];
      if (transform) {
        transformArcs(arcs, scale4, translate3);
      }
      const features = [];
...
function readPointGeometry3(object, scale4, translate3) {
  const coordinates2 = object["coordinates"];
  if (scale4 && translate3) {
    transformVertex(coordinates2, scale4, translate3);
  }
  return new Point_default(coordinates2);
}
function readMultiPointGeometry3(object, scale4, translate3) {
  const coordinates2 = object["coordinates"];
  if (scale4 && translate3) {
    for (let i = 0, ii = coordinates2.length; i < ii; ++i) {
      transformVertex(coordinates2[i], scale4, translate3);
    }
  }
  return new MultiPoint_default(coordinates2);
}
...
function readFeaturesFromGeometryCollection(collection, arcs, scale4, translate3, property, name, options) {
...
function readFeatureFromGeometry(object, arcs, scale4, translate3, property, name, options) {
  let geometry = null;
  const type = object.type;
  if (type) {
    const geometryReader = GEOMETRY_READERS2[type];
    if (type === "Point" || type === "MultiPoint") {
      geometry = geometryReader(object, scale4, translate3);
    } else {
...
function transformArcs(arcs, scale4, translate3) {
  for (let i = 0, ii = arcs.length; i < ii; ++i) {
    transformArc(arcs[i], scale4, translate3);
  }
}
function transformArc(arc, scale4, translate3) {
  let x = 0;
  let y = 0;
  for (let i = 0, ii = arc.length; i < ii; ++i) {
    ...
    vertex[0] = x;
    vertex[1] = y;
    transformVertex(vertex, scale4, translate3);
  }
}
function transformVertex(vertex, scale4, translate3) {
  vertex[0] = vertex[0] * scale4[0] + translate3[0];
  vertex[1] = vertex[1] * scale4[1] + translate3[1];
}
...
//# sourceMappingURL=chunk-V7KRR7T6.js.map
