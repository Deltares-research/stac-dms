...
<template>
    <div class="flex justify-start p-5 ...">
        ...
        <Button v-if="loginDone" @click="navigateTo('/items')" variant="ghost">Register</Button>
        <Button v-if="loginDone" @click="navigateTo('/main')" variant="ghost">Admin</Button>
        <Button @click="navigateTo('/main')" variant="ghost">Storage finder</Button>
    </div>
    <Label class="text-3xl p-4 flex justify-center p-5 ...">Register your data</Label>
    <Card>
        <CardHeader>Data set collection</CardHeader>
        <CardContent>
    <CustomDropDownComponent id="language" :options="frameworks" v-model="value"/>
        </CardContent>
    <Card>
        ...
        <CardContent>
          ...
          !value && 'text-muted-foreground',
                        ...
                        >
                            ...
                            {{ dateDataSet ? df.format(dateDataSet.toDate(getLocalTimeZone())) : "Pick a date" }}
                        </Button>
                    </PopoverTrigger>
                ...
                <CustomDropDownComponent id="datum type" :options="datumTypes" v-model="selectedDatumType"/>
                <Label class="m-2" for="status" >Status</Label>
                <CustomDropDownComponent id="status" :options="statusTypes" v-model="selectedStatus"/>
                <Label class="m-2" for="language" >Language</Label>
                <CustomDropDownComponent id="language" :options="languages" v-model="selectedLanguage"/>
        ...
        <Card>
            ...
            <CardContent>
                <div class="grid grid-cols-[15%_85%] gap-5 p-5">
                    ...
                    <CustomDropDownComponent id="accescontraints" :options="accessConstraints" v-model="selectedAccessConstraint"/>
                </div>
                <div class="grid grid-cols-[15%_85%] gap-5 p-5">
        ...
        <Card>
            ...
            <CardContent>
                <div class="grid grid-cols-[15%_85%] gap-5 p-5">
                    <Label class="m-2" for="spatialreference">Spatial reference system</Label>
                    <CustomDropDownComponent id="spatialreference" :options="spatialReferences" v-model="selectedSpatialReference"/>
        ...
        <CardContent>
            <div class="grid grid-cols-[15%_85%] gap-5 p-5">
                ...
                <Button id="assets" variant="outline" class="w-[280px] justify-start text-left font-normal">
                    <PlusIcon class="w-4 h-4"/>
                    Assets
                </Button>
                <Label for="properties" class="m-2">More properties</Label>
                <Button id="properties" variant="outline" class="w-[280px] justify-start text-left font-normal">
                    <PlusIcon class="w-4 h-4"/>
                    Numerical models
                </Button>
            </div>
