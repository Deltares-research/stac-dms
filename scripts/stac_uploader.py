...
class STACUploader:
    ...
    def create_collection(self, title: str, description: str) -> bool:
        ...
        #return True
        try:
            response = self.session.post(url, json=collection, cookies=self.cookies)
    ...
    def upload_item(self, item: Dict) -> bool:
        ...
        try:
            ...
            if self.use_feature_collection is None:
                ...
                response = self.session.post(url, json=item, cookies=self.cookies)
                ...
                if response.status_code in [200, 201]:
                    ...
                    if 'featurecollection' in error_text and "'feature'" not in error_text:
                        ...
                        response = self.session.post(url, json=feature_collection, cookies=self.cookies)
                        if response.status_code in [200, 201]:
                            return True
    ...
    def delete_item(self, item_id: str) -> bool:
        ...
        try:
            response = self.session.delete(url, cookies=self.cookies)  # Add cookies here
    ...
    def delete_all_items(self, delay: float = 0.1) -> Dict[str, int]:
        ...
        try:
            next_url = url
            failed_pages = 0
            max_failed_pages = 5  # Stop if too many pages fail
            
            while next_url and failed_pages < max_failed_pages:
                print(f"DEBUG: Requesting: {next_url}")
                
                try:
                    response = self.session.get(next_url, cookies=self.cookies, timeout=30, allow_redirects=False)
                    print(f"DEBUG: Status code: {response.status_code}")
                    
                    if response.status_code != 200:
                        print(f"⚠️  Warning: Failed to fetch page (status {response.status_code}), skipping...")
                        failed_pages += 1
                        # Try to continue with next page if we can detect it
                        break  # Or continue to next iteration
                    ...
                    if data.get('type') == 'FeatureCollection':
                        ...
                        print(f"  Fetched {len(features)} items (total: {len(all_items)})")
                        ...
                        if next_link:
                            ...
                            # Force HTTPS if the server returned HTTP
                            if next_url.startswith('http://'):
                                next_url = next_url.replace('http://', 'https://')
                                print(f"DEBUG: Fixed pagination URL to HTTPS: {next_url}")
                        else:
                            next_url = None
                    else:
                    ...
                    print(f"⚠️  Warning: Error fetching page: {e}")
                    failed_pages += 1
                    # Break or continue based on your preference
                    break
            
            if failed_pages >= max_failed_pages:
                print(f"⚠️  Stopped after {failed_pages} failed page requests")
            
            if not all_items:
                print("No items found in collection")
                return {'success': 0, 'failed': 0}
            
            print(f"\nFound {len(all_items)} item(s) to delete\n")
